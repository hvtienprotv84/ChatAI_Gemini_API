    ...
    <div className='conversation-container'>
        ...
        {data &&
          data.map((message, index) => (
            <div className={`message ${message.type === 'inbound' ? 'inbound' : 'outbound'}`} key={index}>
              <strong>{message.type === 'inbound' ? 'Gemini' : 'Bạn'}</strong>
      ...
      <div className='message-input-container'>
        <textarea
          ...
          placeholder='Nhập tin nhắn của bạn ở đây. ví dụ: ReactJS là gì?'
          onChange={handlePromptChange}
          onKeyDown={handleKeyDown}
          ref={textareaRef}
        />
        <Button disabled={!prompt || loading} onClick={handleSendPrompt}>Gửi</Button>
      </div>
    </div>
  )
}
